...
class Collect:
    ...
    def __call__(self, results):
        ...
        if data.get('label') is not None:  # add  
            if isinstance(data['label'], np.ndarray):
                data['label'] = data['label'].astype(np.float32)
            elif isinstance(data['label'], torch.Tensor):
                data['label'] = data['label'].type(torch.float32)
        
        if data.get('text') is not None:  # add by lyn
            if isinstance(data['text'], np.ndarray):
                data['text'] = data['text'].astype(np.float32)
            elif isinstance(data['text'], torch.Tensor):
                data['text'] = data['text'].type(torch.float32)
...
class FormatShape:
    ...
    def __init__(self, input_format, collapse=False):
        ...
        if self.input_format not in ['NCTHW', 'NCHW', 'NCHW_Flow', 'NPTCHW', 'NCTHW_TSN']:
            raise ValueError(
    ...
    def __call__(self, results):
        ...
        Args:
            results (dict): The resulting dict to be modified and passed
                to the next transform in pipeline.
        """
        if 'imgs' not in results and 'img' in results:
            results['imgs'] = results.pop('img')
            results['imgs'] = np.array(results['imgs'])[np.newaxis, ...]
        ...
        if self.input_format == 'NCTHW':
            ...
            # M' x C x L x H x W
            # M' = N_crops x N_clips
        elif self.input_format == 'NCHW':  # 这里的N是num_segments，即采样帧数,不是batch-size
            ...
            # P x M x C x H x W
        elif self.input_format == 'NCTHW_TSN':
            # in TSN segment sampling, clip_len = 1, frame_interval = 1, num_clips = real sample frames
            num_clips = results['num_clips']
            clip_len = results['clip_len']

            imgs = imgs.reshape((-1, clip_len, num_clips) + imgs.shape[1:])
            # N_crops x N_clips x L x H x W x C
            imgs = np.transpose(imgs, (0, 1, 5, 2, 3, 4))
            # N_crops x N_clips x C x L x H x W
            imgs = imgs.reshape((-1, ) + imgs.shape[2:])
