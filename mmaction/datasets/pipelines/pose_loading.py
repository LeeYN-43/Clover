...
@PIPELINES.register_module()
class CenterReSampleFrames:
    """
    从UniformSampleFrames的采样结果进一步随机重采样，
    以概率p从中间抽取指定帧数，以概率1-p均匀抽帧，仅用于训练，搭配UniformSampleFrames使用
    """

    def __init__(self, clip_len, prob=0.5):

        self.clip_len = clip_len
        self.prob = prob

    def __call__(self, results):
        ori_clip_len = results['clip_len']
        frame_inds = results['frame_inds']

        if np.random.rand() > self.prob:  # 不从中间抽帧
            interval = ori_clip_len // self.clip_len
            results['frame_inds'] = frame_inds[0:ori_clip_len:interval]
        else:  # 从中间抽帧
            offset = (ori_clip_len - self.clip_len) // 2
            results['frame_inds'] = frame_inds[offset:ori_clip_len-offset]
        results['clip_len'] = self.clip_len
        return results
