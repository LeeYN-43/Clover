...
class Imgaug:
    ...
    def __init__(self, transforms, num_augmenters=2, cur_magnitude=9, max_magnitude=10):
        import imgaug.augmenters as iaa

        if transforms == 'default':
            self.transforms = self.default_transforms(num_augmenters=num_augmenters,
                                                      cur_magnitude=cur_magnitude,
                                                      max_magnitude=max_magnitude)
        elif transforms == 'deit':
            self.transforms = self.deit_transforms(num_augmenters=num_augmenters,
                                                   cur_magnitude=cur_magnitude,
                                                   max_magnitude=max_magnitude)
        elif transforms == 'double_record':
            self.transforms = self.double_record_transforms(num_augmenters=num_augmenters,
                                                            cur_magnitude=cur_magnitude,
                                                            max_magnitude=max_magnitude)
    ...
    def default_transforms(self, num_augmenters=2, cur_magnitude=9, max_magnitude=10):
    ...
    def deit_transforms(self, num_augmenters=2, cur_magnitude=9, max_magnitude=10):
        """
        区别在于删除了cutout，因为这个在random earasing中已经做了
        """
        # RandAugment hyper params
        # num_augmenters = 2
        # cur_magnitude, max_magnitude = 9, 10
    ...
    def double_record_transforms(self, num_augmenters=2, cur_magnitude=9, max_magnitude=10):
        """
        区别在于删除了cutout，因为这个在random earasing中已经做了，并且删除了rotate，因为这个会被单独设置为必须
        """
        # RandAugment hyper params
        # num_augmenters = 2
        # cur_magnitude, max_magnitude = 9, 10
...
class Normalize:
    ...
    Args:
        ...
        to_rgb (bool): Whether to convert channels from RGB to BGR.
            Default: False.
    ...
    def __call__(self, results):
        ...
        if modality == 'RGB':
            ...
            for img in imgs:
                mmcv.imnormalize_(img, self.mean, self.std, self.to_rgb)
...
def _get_pixels(per_pixel, rand_color, patch_size, dtype=torch.float32):
    ...
    if per_pixel:
        return torch.empty(patch_size, dtype=dtype).normal_().numpy()
    elif rand_color:
        return torch.empty((1, 1, patch_size[0]), dtype=dtype).normal_().numpy()
    else:
        return np.zeros((1, 1, patch_size[0]))


@PIPELINES.register_module()
class RandomErasing:  # add  
    ...
    def _erase(self, img, img_h, img_w):
            ...
            for attempt in range(10):
                ...
                if w < img_w and h < img_h:
                    ...
                    img[top:top + h, left:left + w, :] = _get_pixels(
                        self.per_pixel, self.rand_color, (h, w, 3))
                    break
        return img
...
class ScaleJitter:  # add  
    ...
    def __call__(self, results):
        ...
        new_h = size
        if img_w < img_h:
            new_h = int(math.floor((float(img_h) / img_w) * size))
        else:
            new_w = int(math.floor((float(img_w) / img_h) * size))

        resize = Resize((new_w, new_h),
                        keep_ratio=False,
                        interpolation='bilinear',
                        lazy=False)
        results = resize(results)
        results['keep_ratio'] = True  # 这里其实是keep_ratio的
        return results
...
class MaskingGenerator:
    ...
    """
    def __init__(
            self, window_size, num_masking_patches, min_num_patches=4, max_num_patches=None,
    ...
    def __call__(self, results):
        ...
        while mask_count < self.num_masking_patches:
            ...
            if delta == 0:
                ...
                mask_count += delta
        # mask is (7, 7)
        # adapt to temporal dim -> (1, 7, 7) 
        mask = torch.from_numpy(mask)
        results['v_token_mask'] = mask.unsqueeze(0)

        return results

@PIPELINES.register_module()
class RandomMaskGenerator:
    def __init__(self, input_size=224, mask_patch_size=32, model_patch_size=4, mask_ratio=0.5):
