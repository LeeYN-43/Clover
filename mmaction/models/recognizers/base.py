...
class BaseRecognizer(nn.Module, metaclass=ABCMeta):
    ...
    def __init__(self,
                 ...
                 cls_head=None,
                 neck=None,
                 freeze_stage=None,  # ['backbone'], ['cls_head']
                 freeze_except=[],
                 train_cfg=None,
                 test_cfg=None):
        super().__init__()
        ...
        if not isinstance(cls_head, (list, tuple)):
        ...
        if freeze_stage is not None:
            self._freeze(freeze_stage=freeze_stage, freeze_except=freeze_except)
    ...
    def multi_head(self):
        """bool: whether the recognizer has a cls_head"""
        return hasattr(self, 'head_num')

    def _freeze(self, freeze_stage, freeze_except):
        except_names = []
        freeze_norm_layer = True
        if 'norm_layer' in freeze_except:
            freeze_norm_layer = False
        for n, m in self.named_modules():
            skip = False
            for en in freeze_except:
                if en in n:
                    except_names.append(n)
                    skip = True
                    break
            if skip:
                continue
            for fn in freeze_stage:
                if fn in n:
                    if isinstance(m, (_BatchNorm, _InstanceNorm, nn.LayerNorm, nn.GroupNorm, nn.SyncBatchNorm)):
                    ...
                    break
        print_log(f'freezing stages: {freeze_stage}, except: {except_names}')
    ...
    def average_clip(self, cls_score, num_segs=1):
        ...
        if average_clips not in ['score', 'prob', 'prob_sigmoid', None]:
        ...
        elif average_clips == 'prob_sigmoid':
            cls_score = torch.sigmoid(cls_score).mean(dim=1)
    ...
    def _parse_losses(losses):
        ...
        # the name of you loss mush have "loss"
    ...
    def forward(self, imgs=None, label=None, return_loss=True, **kwargs):
        ...
        if return_loss:
            ...
            if self.blending is not None:  # blending = mix strategy
    ...
    def train_step(self, data_batch, optimizer, **kwargs):
        ...
        # print("single gpu train step batch size:", imgs.shape[0])
