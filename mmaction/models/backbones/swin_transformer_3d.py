...
class SwinTransformer3D(nn.Module):
    ...
    def __init__(self,
        ...
        self.patch_embed = PatchEmbed3D(
            patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim, stride=stride,
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            ...
            mask_tokens = self.mask_token.expand(B, -1, T, H, W)
            w = mask.unsqueeze(-1).unsqueeze(-3).expand(-1, T, -1, H // mh, -1, W // mw).type_as(mask_tokens)
            w = w.flatten(2,3).flatten(3,4).unsqueeze(1)
...
def compute_mask(D, H, W, window_size, shift_size, device, dtype):
    # dtype is to fix the bug when inferencing with half precision
...
class PatchEmbed3D(nn.Module):
    ...
    """
    def __init__(self, patch_size=(2,4,4), in_chans=3, embed_dim=96, norm_layer=None, stride=(2,4,4)):
        super().__init__()
        self.patch_size = patch_size
