...
def classwise_accuracy(scores, labels):
    ...
    classwise_acc = [hit / cnt if cnt else 0.0 for cnt, hit in zip(cls_cnt, cls_hit)]
...
def mean_average_precision(scores, labels):
    ...
    """
    results = []
    # scores type: <class 'list'>, labels type: <class 'list'>
    scores = np.stack(scores).T  # (num_images, num_classes) -> (num_classes, num_images)
    labels = np.stack(labels).T  # (num_images, num_classes) -> (num_classes, num_images)
    # scores shape: (13, 192), labels shape: (13, 192)

    for score, label in zip(scores, labels):
        # score shape: (num_classes,), label shape: (num_classes,)
        precision, recall, _ = binary_precision_recall_curve(score, label)
        ap = -np.sum(np.diff(recall) * np.array(precision)[:-1])
        results.append(ap)
...
def classwise_average_precision(scores, labels):
    """
    average precision for multi-label recognition.
    """
    results = []
    # scores type: <class 'list'>, labels type: <class 'list'>
    ...
    for score, label in zip(scores, labels):
        # score shape: (192,), label shape: (192,)
        precision, recall, _ = binary_precision_recall_curve(score, label)
        ap = -np.sum(np.diff(recall) * np.array(precision)[:-1])
        results.append(ap)
    return results
...
def specify_precision_recall(scores, labels, prec_spec=0.9, rec_spec=None):
    """
    指定precision下的recall或者指定recall下的precision
    """
    def get_nearest_value(precision, prec_spec, recall, thresholds):
        pre_diff = np.abs(precision - prec_spec)
        distinct_inds = np.where(np.diff(pre_diff))[0]
        if len(distinct_inds):
            pre_diff = pre_diff[distinct_inds]
            precision, recall, thresholds = precision[distinct_inds], recall[distinct_inds], thresholds[distinct_inds]
            index = np.argmin(pre_diff)
        else:
            index = pre_diff.shape[0] - 1  # 如果所有的value都一样，取最后一个index
        pre, rec, thr = precision[index], recall[index], thresholds[index]
        return pre, rec, thr

    if prec_spec is not None:
        mode = 'precision'
        mode_value = prec_spec
        assert rec_spec is None
    ...
    for i, (score, label) in enumerate(zip(scores, labels)):
        ...
        desc_score_indices = np.argsort(score, kind='mergesort')[::-1]  # 归并排序，从大到小
        score = score[desc_score_indices]
        label = label[desc_score_indices]
        # There may be ties in values, therefore find the `distinct_value_inds`
        distinct_value_inds = np.where(np.diff(score))[0]
        threshold_inds = np.r_[distinct_value_inds, label.size - 1]
        # accumulate the true positives with decreasing threshold
        tps = np.cumsum(label)[threshold_inds]
        fps = 1 + threshold_inds - tps
        thresholds = score[threshold_inds]
        ...
        if mode == 'precision':
            pre, rec, thr = get_nearest_value(precision, prec_spec, recall, thresholds)
        else:
            pre, rec, thr = get_nearest_value(recall, rec_spec, precision, thresholds)
        results.append((pre, rec, thr))
        # if not any((pre, rec, thr)):
        #     zero_ind.append(i)
    # ap, ar, _ = np.mean(np.delete(np.array(results), zero_ind, axis=0), axis=0)
    ap, ar, _ = np.mean(np.array(results), axis=0)
    return mode, mode_value, ap, ar, results


def specify_threshold(scores, labels, thr=0.5, average=False):
    """
    指定阈值计算precision和recall
    """
    ...
    scores_bin = (scores >= thr).astype(np.int8)  # binary
    sub_matrix = labels - scores_bin
    fps = (sub_matrix == -1).sum(axis=0)
    tps = ((sub_matrix == 0) * labels).sum(axis=0)
    fns = (sub_matrix == 1).sum(axis=0)
    classwise_precision = tps / (tps + fps)
    classwise_recall = tps / (tps + fns)
    if not average:
        return classwise_precision, classwise_recall
    else:
        return classwise_precision.mean(), classwise_recall.mean()


def choice_thres(scores, labels, prec_thres=0.9):
    """
    用于主题分类计算precision=0.9的阈值
    """
    label_num = scores[0].shape[0]
    label_pr = [(0, 0) for _ in range(0, label_num)]
    tp_dict = {}
    fp_dict = {}
    score_dict = {}
    positive_dict = {}
    ...
    for label in range(0, label_num):
        ...
        fp_dict[label] = []
        score_dict[label] = []
        positive_dict[label] = 0
        choice_thres[label] = 1.0

    # get tp & fp
    for prediction, gt in zip(scores, labels):
        for label in range(0, label_num):
            if gt[label] == -1:
                continue
            if gt[label] == 1:
                positive_dict[label] += 1
                tp_dict[label].append(1)
                fp_dict[label].append(0)
            else:
                tp_dict[label].append(0)
                fp_dict[label].append(1)
            score_dict[label].append(prediction[label])

    # choice thres
    for label in score_dict:
        tps = np.asarray(tp_dict[label], dtype=np.float32)
        fps = np.asarray(fp_dict[label], dtype=np.float32)
        positive_num = positive_dict[label]
        scores = np.asarray(score_dict[label])
        if positive_num:
            sort_idxes = np.argsort(-scores)
            tps = np.cumsum(tps[sort_idxes])
            fps = np.cumsum(fps[sort_idxes])
            scores = scores[sort_idxes]
            precision = tps / np.maximum(tps + fps, 1e-9)
            recall = tps / max(float(positive_num), 1e-9)
            precision = np.concatenate(([1], precision, [0]))
            recall = np.concatenate(([0], recall, [1]))
            scores = np.concatenate(([1], scores, [0]))
            for idx in range(precision.size - 1, 0, -1):
                precision[idx - 1] = np.maximum(precision[idx - 1],
                                                precision[idx])
            score_idx = np.max(np.where(precision >= prec_thres)[0])
            label_pr[label] = (precision[score_idx], recall[score_idx])
            choice_thres[label] = scores[score_idx]
    return choice_thres
...
def acc_for_msrvtt_mc(video_embd=None, text_embd=None, label=None, use_sim=False, texts=None):
    '''
    add by lyn
    MSRVTT and LSMDC multiple choice
    args:
        video_embd: N, D
        text_embd:  N*5, D
    '''
    b_v, _ = video_embd.shape
    if use_sim:
        print("------- aaa ----------")
        scores = sim_matrix(video_embd, text_embd)
    else:
        print("------- bbb ----------")
        #  2990, 2990*5
        scores = np.dot(video_embd, text_embd.T)
        scores = torch.from_numpy(scores)
    b_v, b_t = scores.shape
    ans_num = b_t // b_v
    scores = scores.reshape((b_v, b_v, ans_num))
    ...
    with open("gt.npy", 'wb') as f:
        np.save(f, label)
    ans = torch.argmax(ans_diag, dim=-1)
    acc =  (ans.cpu() == label).float().mean().item()
    metrics = {}
    metrics['acc'] = acc
    return metrics 


def recall_for_video_text_retrieval(video_embd=None, text_embd=None, input_scores=None, use_sim=False, texts=None):
    '''
    add by lyn
    ...
    if input_scores is not None:
        ...
        scores = sim_matrix(text_embd, video_embd).cpu().numpy()
    else:
        from mmaction.utils import normalize_fn
        text_norm = normalize_fn(text_embd)
        video_norm = normalize_fn(video_embd)
        scores = np.dot(text_norm, video_norm.T)

        
    sx = np.argsort(-scores, axis=1)
    gt = np.arange(len(scores))
    ind = np.where(sx == gt[:, None])[1]
    metrics = {}
    metrics['Recall@1'] = float(np.sum(ind == 0)) / len(ind) * 100  # 百分比
    metrics['Recall@5'] = float(np.sum(ind < 5)) / len(ind) * 100
    metrics['Recall@10'] = float(np.sum(ind < 10)) / len(ind) * 100
...
def recall_for_video_text_retrieval_varied(video_embd, text_embd, tid):
    '''
    add by lyn
    compute video-text retrieval R1 R5 R10 medR
    one video correspond to multiple texts
    e.g.  MSRVTT full test set (2990 videos -> 2990 * 20 texts)
          MSVD test set (670 videos -> 670 * xx = 27763 texts) every video map to different num texts
    follow Use What You Have: Video Retrieval Using Representations From Collaborative Experts (BMVC 2019)
    some reporting the minimum rank among all valid text descriptions for a given video query
    args:
        video_embd: N, D
        text_embd:  N, D
        vid: video_index
        tid: text correspond video_index
    '''
    scores = sim_matrix(text_embd, video_embd).cpu().numpy()
    # scores = np.dot(text_embd, video_embd.T)
    sx = np.argsort(-scores, axis=1)
    gt = []
    fname_list = []
    for id, fname in enumerate(tid):
    ...
    # vid_scores = {}
    # for idx, t2vid in enumerate(fname_list):
    #     if t2vid not in vid_scores:
    #         vid_scores[t2vid] = ind[idx]
    #     else:
    #         vid_scores[t2vid] = min(vid_scores[t2vid], ind[idx])
    
    # final_ind = np.array([s for s in vid_scores.values()])

    # metrics = {}
    # metrics['Recall@1'] = float(np.sum(final_ind == 0)) / len(final_ind) * 100  # 百分比
    # metrics['Recall@5'] = float(np.sum(final_ind < 5)) / len(final_ind) * 100
    # metrics['Recall@10'] = float(np.sum(final_ind < 10)) / len(final_ind) * 100
    # metrics['MR'] = np.median(final_ind) + 1
    # not in use
    # worst 20 sample
    # max_idx = np.argmax(scores, axis=1)
    # max_idx_sort = np.argsort(max_idx)
    # error = []
    # for ex_idx in range(999, 999-20, -1):
    #     error.append((texts[max_idx_sort[ex_idx]], max_idx[max_idx_sort[ex_idx]]))
    #     print(texts[max_idx_sort[ex_idx]], "index:", max_idx[max_idx_sort[ex_idx]])
    # metrics["error"] = error
    # print(metrics["error"])
    return metrics
...
def recall_at_precision(scores, labels, prec_thres=0.9):
    """
    用于主题分类计算precision=prec_thres的召回
    """
    label_thres = choice_thres(scores, labels, prec_thres)
    label_num = scores[0].shape[0]
    tp_dict = {}
    fp_dict = {}
    positive_dict = {}
    ...
    for prediction, gt in zip(scores, labels):
        for label in range(0, label_num):
            if gt[label] == -1:
                continue
            positive_dict[label] += gt[label]
            if prediction[label] >= label_thres[label]:
                if gt[label]:
                    tp_dict[label] += 1
                else:
                    fp_dict[label] += 1
    ...
    threshold_list = []
    for label in range(0, label_num):
        threshold_list.append(label_thres[label])
        positive_num = positive_dict[label]
        precision = tp_dict[label] / max(float(tp_dict[label] + fp_dict[label]), 1e-9)
        recall = tp_dict[label] / max(float(positive_num), 1e-9)
        precision_list.append(precision)
        recall_list.append(recall)
        if precision >= 0:  # 0.5 and recall >= 0.3:
            avg_precision += precision
            avg_recall += recall
            avg_labels += 1
    avg_recall /= float(avg_labels)
    avg_precision /= float(avg_labels)
    return avg_recall, avg_precision, precision_list, recall_list, threshold_list
...
def binary_precision_recall_curve(y_score, y_true):
    ...
    y_true = (y_true == 1)
    # sort scores and corresponding truth values
    desc_score_indices = np.argsort(y_score, kind='mergesort')[::-1]  # 归并排序，从大到小
