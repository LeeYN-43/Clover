_base_ = [
    '../_base_/datasets_local/tgif_action_mc.py',
...
machines = 1
num_frames = 8
base_lr = 5e-6 / (videos_per_gpu * num_gpus * machines)
multi_class = False
...
data = dict(
    ...
    val=dict(batch_sizes=[videos_per_gpu]),
    )
evaluation = dict(interval=1, metrics=['video_qa_mc'], gpu_collect=True, test_fn='use_itm_head_fn')
# optimizer
optimizer = dict(
...
lr_config = dict(policy='CosineAnnealing', min_lr_ratio=0, by_epoch=True,
                 warmup='linear', warmup_iters=10, warmup_ratio=0.0001, warmup_by_epoch=True)
total_epochs = 100
